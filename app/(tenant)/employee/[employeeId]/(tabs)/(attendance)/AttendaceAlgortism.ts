...
export function OtMoneyCalculator(
  grossSalary: number,
  hours: number,
  isWeekend?: boolean
) {
  ...
  } else return Number(((grossSalary / 176) * hours * 2.5).toFixed(2));
}
...
export function getMinMaxDates(dates: Date[]): {
  minDate: Date;
  maxDate: Date;
} {
  ...
  const maxDate = new Date(
    new Date(sortedDates[sortedDates.length - 1]).getTime() - 180 * 60000
  );
...
export const OtCalculate = (
  minDate: Date,
  maxDate: Date,
  holyDay: boolean,
  weekend: boolean,
  workingHourIn?: string,
  workingHourOut?: string
) => {
  if (holyDay) {
    ...
    return {
      fullDayOt:{holyday: OtNumber},
    };
  } else if (!(workingHourIn && workingHourOut)) {
    if (weekend) {
      const diffInMinutes = Math.floor(
        (maxDate.getTime() - minDate.getTime()) / (1000 * 60)
      );
      ...
      const OtNumber = diffInHours + Number((remainingMinutes / 60).toFixed(2));
      return {
        fullDayOt:{weekend: OtNumber},
      };
    } else {
      const timeBefore10Pm: Date = new Date(minDate.setHours(22, 0, 0, 0));
      const timeAfter10PM = new Date(minDate.setHours(22, 1, 0, 0));
      const ot: { before10Pm?: number; after10Pm?: number } = {};
      const diffInMinutes = Math.floor(
        (maxDate.getTime() - minDate.getTime()) / (1000 * 60)
      );
      const diffInHours = Math.floor(diffInMinutes / 60);
      const remainingMinutes = diffInMinutes % 60;
      const OtNumber = diffInHours + Number((remainingMinutes / 60).toFixed(2));
      ot.before10Pm = OtNumber;

      if (maxDate > timeBefore10Pm) {
        const diffInMinutes = Math.floor(
          (maxDate.getTime() - timeBefore10Pm.getTime()) / (1000 * 60)
        );
        const diffInHours = Math.floor(diffInMinutes / 60);
        const remainingMinutes = diffInMinutes % 60;
        const OtNumber =
          diffInHours + Number((remainingMinutes / 60).toFixed(2));
        ot.after10Pm = OtNumber;
      }
      return { fullDayOt:{...ot} };
    }
  } else {
    const workingHourInTime = workingHourIn.split(":");
    const workingHourOutTime = workingHourOut.split(":");
    const workingHourInDate = new Date(
      minDate.setHours(
        Number(workingHourInTime[0]),
        Number(workingHourInTime[1]),
        0,
        0
      )
    );
    const workingHourOutDate = new Date(
      minDate.setHours(
        Number(workingHourOutTime[0]),
        Number(workingHourOutTime[1]),
        0,
        0
      )
    );
    ...
    if (weekend) {
      const diffInMinutes = Math.floor(
        (minDate.getTime() - minDate.getTime()) / (1000 * 60)
      );
      const diffInHours = Math.floor(diffInMinutes / 60);
      const remainingMinutes = diffInMinutes % 60;
      const OtNumber = diffInHours + Number((remainingMinutes / 60).toFixed(2));
